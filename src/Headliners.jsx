  ...
  const beginYearEndEffectsPhase = () => {
    ...
    players.forEach(p => {
      ...
      (pd.stageArtists || []).forEach(sa => sa.forEach(a => {
        ...
        // Coldplay: "For every 10 tickets before Year End: +1 ticket"
        else if (eff.includes("for every 10 tickets before year end")) {
          const currentTix = pd.tickets || 0;
          const bonus = Math.floor(currentTix / 10);
          if (bonus > 0) effects.push({ artist: a, type: "autoVPTix", desc: `${currentTix} tickets / 10 = +${bonus} tickets`, autoVP: 0, autoTix: bonus });
        }
        // Foo Fighters: "For every 3 amenities you own: +1 ticket"
        else if (eff.includes("for every 3 amenities you own")) {
          const am = pd.amenities || {};
          const total = (am.campsite || 0) + (am.security || 0) + (am.catering || 0) + (am.portaloo || 0);
          const bonus = Math.floor(total / 3);
          if (bonus > 0) effects.push({ artist: a, type: "autoVPTix", desc: `${total} amenities / 3 = +${bonus} tickets`, autoVP: 0, autoTix: bonus });
        }

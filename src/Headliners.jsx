  ...
  function applyEffect(artist, pid, times = 1, stageIdx = -1, viaAgent = false, viaTempt = false, prevArtist = null, prevArtistPid = null, slotCount = 0) {
    ...
    for (let t = 0; t < times; t++) {
      ...
      if (el.includes("+4 ticket sales")) {
        ...
        // v193 Lady Gaga special: base +4 fires on any play; if she was played via a tempt,
        // she pays out +3 more tickets on top (total +7). This name-based handler exists so
        // her effect string can stay a simple "+4 ticket sales" that the base regex handles,
        // with the tempt-only bonus stacked here.
        if (artist.name === "Lady Gaga" && viaTempt) {
          logTicketGain(pid, 3, `${artist.name} tempt bonus (+3)`);
          setPlayerData(p => ({ ...p, [pid]: { ...p[pid], bonusTickets: (p[pid].bonusTickets || 0) + 3 } }));
          addLog("Effect", `${artist.name}: +3 ticket sales (tempt bonus, total +7)`);
          showFloatingBonus("+3 🎟️ Lady Gaga tempt!", "#f472b6");
        }
      }
